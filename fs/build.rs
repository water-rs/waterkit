...
fn build_apple() {
    ...
    {
        // Avoid unused var warnings on non-apple
        let _ = out_dir;
    }
}

fn build_android() {
    const KOTLIN_FILE_RELATIVE_PATH: &str = "src/sys/android/FsHelper.kt";
    ...
    // We name it classes.dex by default in the out_dir, or we can name it specifically.
    // The mod.rs snippet I wrote uses `waterkit_fs.dex` inside the cache dir AFTER extraction from the embedded bytes.
    // But `include_bytes!` in `mod.rs` reads `classes.dex` from `OUT_DIR`.
    // So we should just let D8 output to `classes.dex` in `out_dir`.
    ...
    java_run
        ...
        .arg(&android_jar_path)
        .arg("--output")
        .arg(&out_dir); // this will produce classes.dex in out_dir
