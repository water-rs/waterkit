...
fn get_helper_class<'a>(env: &'a mut JNIEnv) -> Result<JClass<'a>, BiometricError> {
    let class_loader = CLASS_LOADER.get()
        .ok_or(BiometricError::PlatformError("Class loader not initialized".into()))?;
...
pub unsafe extern "system" fn Java_waterkit_biometric_BiometricHelper_onResult(
    ...
    if success != 0 {
        ...
        let error_str: String = env.get_string(&error_msg).map(|s| s.into()).unwrap_or_else(|_| "Unknown JNI error".into());
...
pub async fn authenticate(_reason: &str) -> Result<(), BiometricError> {
    Err(BiometricError::PlatformError("Android requires authenticate_with_context".into()))
}

// Public API extending the standard one
pub fn authenticate_with_context(env: &mut JNIEnv, context: &JObject, reason: &str) -> Result<tokio::sync::oneshot::Receiver<Result<(), BiometricError>>, BiometricError> {
    ...
    let reason_jstr = env.new_string(reason).map_err(|e| BiometricError::PlatformError(format!("new_string: {e}")))?;
    ...
    env.call_static_method(
        ...
        &[JValue::Object(context), JValue::Object(&reason_jstr), JValue::Long(sender_ptr)]
    ).map_err(|e| {
