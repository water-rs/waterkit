...
pub async fn set(_service: &str, _account: &str, _password: &str) -> Result<(), SecretError> {
    ...
    Err(SecretError::System("On Android, use `waterkit_secret::android::set_with_context`".into()))
}

/// Retrieve a secret (stub, use `get_with_context`).
pub async fn get(_service: &str, _account: &str) -> Result<String, SecretError> {
    Err(SecretError::System("On Android, use `waterkit_secret::android::get_with_context`".into()))
}

/// Delete a secret (stub, use `delete_with_context`).
pub async fn delete(_service: &str, _account: &str) -> Result<(), SecretError> {
    Err(SecretError::System("On Android, use `waterkit_secret::android::delete_with_context`".into()))
} 
...
pub fn set_with_context(
    ...
    let prefs_name = env.new_string("waterkit_secrets")
    ...
    ).map_err(|e| SecretError::System(e.to_string()))?.l()
    .map_err(|e| SecretError::System(e.to_string()))?;
    ...
    let key = env.new_string(key_str).map_err(|e| SecretError::System(e.to_string()))?;
    let val = env.new_string(password).map_err(|e| SecretError::System(e.to_string()))?;
...
pub fn get_with_context(
    ...
    let val_str: String = env.get_string(&val_jstr)
        .map_err(|e| SecretError::System(e.to_string()))?
        .into();
