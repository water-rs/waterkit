...
private let logger = Logger(subsystem: "dev.waterui", category: "WaterKitVideoPlayerPiP")
...
typealias SetExternalRenderingFn =
  @convention(c) (
    UnsafeMutableRawPointer?,
    Bool
  ) -> Void
...
private final class PictureInPictureCommandChannel {
  private let condition = NSCondition()
  private var commands: [WaterKitPictureInPictureCommandRecord] = []
  private var closed = false

  func send(_ command: WaterKitPictureInPictureCommandRecord) {
    condition.lock()
  ...
  func wait() -> WaterKitPictureInPictureCommandRecord? {
    condition.lock()
    defer { condition.unlock() }
...
private final class PictureInPictureCommandRegistry {
  static let shared = PictureInPictureCommandRegistry()

  private let lock = NSLock()
  private var channels: [UInt64: PictureInPictureCommandChannel] = [:]

  func open(hostId: UInt64) -> PictureInPictureCommandChannel {
    lock.lock()
    defer { lock.unlock() }
    if let channel = channels[hostId] {
      return channel
    }
    let channel = PictureInPictureCommandChannel()
    channels[hostId] = channel
    return channel
  }

  func wait(hostId: UInt64) -> WaterKitPictureInPictureCommandRecord? {
    lock.lock()
    let channel = channels[hostId]
    lock.unlock()
    return channel?.wait()
  }

  func close(hostId: UInt64) {
    lock.lock()
    let channel = channels.removeValue(forKey: hostId)
    lock.unlock()
    channel?.close()
  }
}
...
private final class PictureInPictureHostRegistration {
  ...
  let commandChannel: PictureInPictureCommandChannel
  ...
  init(
    ...
    self.commandChannel = PictureInPictureCommandRegistry.shared.open(hostId: hostId)
  }
}
  ...
  AVPictureInPictureSampleBufferPlaybackDelegate
{
  ...
  private func enqueueFrame() {
    ...
    if displayLayer.status == .failed {
      logger.warning(
        "Sample buffer display layer failed, flushing for PiP host \(self.host.hostId)")
  ...
  func pictureInPictureControllerDidStartPictureInPicture(
    ...
    host.commandChannel.send(
      WaterKitPictureInPictureCommandRecord(kind: .active, valueSeconds: 0)
    )
  }
  ...
  func pictureInPictureControllerDidStopPictureInPicture(
    ...
    host.commandChannel.send(
      WaterKitPictureInPictureCommandRecord(kind: .inactive, valueSeconds: 0)
    )
...
private final class PictureInPictureManager {
  ...
  func unregisterHost(hostId: UInt64) {
    ...
    PictureInPictureCommandRegistry.shared.close(hostId: hostId)
  }
  ...
  func syncHostState(
    ...
    let host =
      hosts[hostId]
      ?? PictureInPictureHostRegistration(
        hostId: hostId,
        userData: nil,
...
func waterkitVideoApplePipBridgeRegisterHost(
  _ hostId: UInt64,
...
func waterkitVideoApplePipBridgeUnregisterHost(_ hostId: UInt64) {
  runOnMainAsync {
    if #available(iOS 15.0, macOS 12.0, *) {
...
func waterkitVideoApplePipBridgeSyncHostState(
  _ hostId: UInt64,
  _ active: Bool,
  _ playing: Bool,
...
func waterkitVideoApplePipBridgeEnter(_ hostId: UInt64) -> Int32 {
  runOnMain {
    if #available(iOS 15.0, macOS 12.0, *) {
...
@_cdecl("waterkit_video_apple_pip_bridge_open_command_channel")
func waterkitVideoApplePipBridgeOpenCommandChannel(_ hostId: UInt64) {
  _ = PictureInPictureCommandRegistry.shared.open(hostId: hostId)
}

@_cdecl("waterkit_video_apple_pip_bridge_close_command_channel")
func waterkitVideoApplePipBridgeCloseCommandChannel(_ hostId: UInt64) {
  PictureInPictureCommandRegistry.shared.close(hostId: hostId)
}

@_cdecl("waterkit_video_apple_pip_bridge_wait_command_kind")
func waterkitVideoApplePipBridgeWaitCommandKind(
  _ hostId: UInt64,
  _ kindOut: UnsafeMutablePointer<Int32>?,
  _ valueSecsOut: UnsafeMutablePointer<Double>?
) {
  let command = PictureInPictureCommandRegistry.shared.wait(hostId: hostId)
