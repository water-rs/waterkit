...
impl PerformanceStats {
    ...
    fn print_summary(&self, elapsed: Duration) {
        ...
        println!("Dropped:        {} (queue full: {})", self.dropped_frames, self.queue_full_drops);
        println!("Actual FPS:     {:.2}", actual_fps);
        println!("Output size:    {:.2} MB", self.total_bytes as f64 / 1_000_000.0);
        println!("Bitrate:        {:.2} Mbps", bitrate_mbps);
        println!("\n-- Capture Times --");
        println!("  Average:      {:.2} ms", self.avg(&self.capture_time_ms));
        println!("  P95:          {:.2} ms", self.percentile(&self.capture_time_ms, 95));
        println!("  Max:          {:.2} ms", self.max(&self.capture_time_ms));
        println!("\n-- Encode Times --");
        println!("  Average:      {:.2} ms", self.avg(&self.encode_time_ms));
        println!("  P95:          {:.2} ms", self.percentile(&self.encode_time_ms, 95));
        ...
        println!("  Max theoretical FPS (sequential): {:.1}", 1000.0 / total_pipeline);
        println!("  Max theoretical FPS (pipelined):  {:.1}", 1000.0 / self.avg(&self.capture_time_ms).max(self.avg(&self.encode_time_ms)));
        println!("=================================================");
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (tx, rx): (mpsc::SyncSender<CapturedFrame>, Receiver<CapturedFrame>) = mpsc::sync_channel(BUFFER_SIZE);
    ...
    while start_time.elapsed() < RECORDING_DURATION + Duration::from_millis(500) {
        ...
        if last_progress_print.elapsed() > Duration::from_secs(1) {
            ...
            let progress = (elapsed.as_secs_f64() / RECORDING_DURATION.as_secs_f64() * 100.0) as usize;
            let bar = "█".repeat((progress / 2).min(50));
            let remaining = " ".repeat(50 - (progress / 2).min(50));
            print!("\rProgress: [{}{}] {}%  FPS: {:.1}  Size: {:.1}MB  ", 
                   bar, remaining, progress.min(100),
