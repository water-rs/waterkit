...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Screen Capture Optimization Test ({} iterations each)\n", ITERATIONS);
    ...
    println!("=== Test 1: capture_screen_raw (per-call Screen::all) ===");
    {
        ...
        println!("Total: {:?}, Avg capture: {:?}/frame, FPS: {:.1}\n",
                 total, total_capture / ITERATIONS as u32, ITERATIONS as f64 / total.as_secs_f64());
    }
    ...
    println!("=== Test 3: ScreenCapturer + H.265 Encode ===");
    {
        ...
        let total = start.elapsed();
        println!("Total: {:?}", total);
        println!("Avg capture: {:?}, Avg encode: {:?}", total_capture / 50, total_encode / 50);
    ...
    println!("=== Test 4: SCKCapturer 120fps Capture (Zero-Copy IOSurface) ===");
    {
        match waterkit_screen::SCKCapturer::new() {
            Some(capturer) => {
                ...
                println!("Callback frames (frame_count): {} ({:.1} fps)", frame_count, sck_fps);
                println!("IOSurface frames (sequence): {} ({:.1} fps)", unique_frames, ios_fps);
                println!("IOSurface pointer reads: {} ({:.0}/sec)", iosurface_reads, iosurface_reads as f64 / total.as_secs_f64());
