//! Build script for waterkit-biometric.
//!
...
fn build_apple() {
    ...
    let swift_source = manifest_dir.join("src/sys/apple/Biometric.swift");
    ...
    let pkg_h = out_dir.join("waterkit-biometric/waterkit-biometric.h");
    ...
    let gen_swift = out_dir.join("waterkit-biometric/waterkit-biometric.swift");
    let combined_swift = out_dir.join("CombinedBiometric.swift");
    ...
    let impl_content = fs::read_to_string(&swift_source).expect("Failed to read Biometric.swift");
    ...
    let obj_file = out_dir.join("Biometric.o");
    ...
    let lib_file = out_dir.join("libBiometric.a");
    ...
    println!("cargo:rustc-link-lib=static=Biometric");
...
fn build_android() {
    const KOTLIN_FILE_RELATIVE_PATH: &str = "src/sys/android/BiometricHelper.kt";
